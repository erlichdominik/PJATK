...
public class Currency {
	...
	public Integer universalValue(Integer amount) {
		Double val = this.rate * amount;
		String _val = String.valueOf(val).replace(".", "");
	...
	*/
	public Integer valueInThisCurrency(Integer amount, Currency othercurrency) {
		// SEK, amount = 1000, other currency rate = 0.5, our currency rate = 0.3
		// change amount to global currency
		// amount * othercurreny.rate
		// change global amount to our currency
		// globalammount * this.rate
		Double _amount = Money.ConvertAmountToDouble(amount);
		Double globalCurrencyAmount = _amount * othercurrency.getRate();
		Double returnVal =  globalCurrencyAmount * this.rate;
		String _returnVal = String.valueOf(returnVal).replace(".", "");
		return Integer.parseInt(_returnVal);
	}
}
